...
int PaymentScreen::remainingSec(uint32_t now) {
    return static_cast<int>(RicCheckout::remainingSeconds(now,_startedMs,static_cast<uint32_t>(_ttlSec)*1000U));
}

void PaymentScreen::draw(TFT_eSPI& tft,const String& bolt11,long sats,const String& fiat,int ttlSec) {
    if (bolt11!=_timedBolt11) {
        _timedBolt11=bolt11;
        _ttlSec=ttlSec>0 ? ttlSec : 600;
        _startedMs=millis();
    }
    ...
    tft.setTextColor(COL_MUTED,COL_CARD); tft.setTextDatum(MC_DATUM);
...
void PaymentScreen::drawNfcHint(TFT_eSPI& tft,int phase) {
    ...
    tft.setTextFont(FONT_SMALL); tft.setTextDatum(TC_DATUM);
    tft.setTextColor(COL_TEXT,COL_BG);
    // Protocol stage labels are bounded UI copy, never raw server secrets.
    tft.drawString(_stage,INFO_X+INFO_W/2,166);
}
