...
import { MIN_PAYOUT_SATS, listPartnerEarnings, listPartnerPayouts, partnerBalance, requestPartnerPayout, setPayoutAddress } from "../core/money/partnerPayouts.js";
...
export async function createPartner(name:string,handle:string,region:string,password:string,status="active"){if(!name||!handle||password.length<12)throw Error("name, handle, and password of at least 12 characters are required");const [p]=await db.insert(partnerAccountsTable).values({name,handle:handle.trim().toLowerCase(),region,passwordHash:digest(password),status}).returning();await audit(p.id,"partner.created",{handle:p.handle,region:p.region});return {id:p.id,name:p.name,handle:p.handle,region:p.region,status:p.status};}
export async function setPartnerPassword(handle:string,password:string){if(password.length<12)throw Error("Password must be at least 12 characters");const [p]=await db.update(partnerAccountsTable).set({passwordHash:digest(password)}).where(eq(partnerAccountsTable.handle,handle.trim().toLowerCase())).returning({id:partnerAccountsTable.id,handle:partnerAccountsTable.handle});if(!p)throw Error("Partner not found");await audit(p.id,"partner.password_reset",{});return p.handle;}
export async function setPartnerStatus(handle:string,status:string){if(!["active","pending","suspended"].includes(status))throw Error("status must be active, pending, or suspended");const [p]=await db.update(partnerAccountsTable).set({status}).where(eq(partnerAccountsTable.handle,handle.trim().toLowerCase())).returning({id:partnerAccountsTable.id,handle:partnerAccountsTable.handle,status:partnerAccountsTable.status});if(!p)throw Error("Partner not found");await audit(p.id,"partner.status_set",{status});return p;}
...
export async function handlePartnerRoute(req:IncomingMessage,res:ServerResponse,u:URL):Promise<boolean>{
 ...
 if(req.method==="GET"&&u.pathname.match(/^\/api\/partner\/[^/]+\/dashboard$/)){const id=u.pathname.split("/")[3];if(!pid||pid!==id)return json(res,401,{error:"Partner authentication required"});
  const [partner]=await db.select().from(partnerAccountsTable).where(eq(partnerAccountsTable.id,id));
  if(!partner)return json(res,404,{error:"Partner not found"});
  const [balance,earnings,payouts,devices]=await Promise.all([
   partnerBalance(id),listPartnerEarnings(id,200),listPartnerPayouts(id,50),
   db.select({
    mac:posboxDevicesAttributionTable.mac,firstSeenAt:posboxDevicesAttributionTable.firstSeenAt,lastSeenAt:posboxDevicesAttributionTable.lastSeenAt,
    earnedSats:sql<number>`coalesce((select sum(e.fee_share_sats) from partner_earnings e where e.partner_id=${id} and e.device_mac=${posboxDevicesAttributionTable.mac}),0)::int`,
    payments:sql<number>`(select count(*) from partner_earnings e where e.partner_id=${id} and e.device_mac=${posboxDevicesAttributionTable.mac})::int`,
   }).from(posboxDevicesAttributionTable).where(eq(posboxDevicesAttributionTable.partnerId,id)).orderBy(desc(posboxDevicesAttributionTable.firstSeenAt)),
  ]);
  return json(res,200,{partner:{id:partner.id,name:partner.name,handle:partner.handle,region:partner.region,lightningAddress:partner.lightningAddress},balance:{...balance,minPayoutSats:MIN_PAYOUT_SATS},earnings,payouts,devices});}
 if(req.method==="PUT"&&u.pathname.match(/^\/api\/partner\/[^/]+\/payout-address$/)){const id=u.pathname.split("/")[3];if(!pid||pid!==id)return json(res,401,{error:"Partner authentication required"});try{const v=await body(req);const r=await setPayoutAddress(id,String(v.address??""));return r.ok?json(res,200,{address:r.address}):json(res,r.status,{error:r.error});}catch{return json(res,400,{error:"Invalid request"});}}
 if(req.method==="POST"&&u.pathname.match(/^\/api\/partner\/[^/]+\/payouts$/)){const id=u.pathname.split("/")[3];if(!pid||pid!==id)return json(res,401,{error:"Partner authentication required"});const r=await requestPartnerPayout(id);return r.ok?json(res,201,{payout:r.payout}):json(res,r.status,{error:r.error});}
