...
export async function handleCardsRoute(req: IncomingMessage, res: ServerResponse, u: URL, account: { id: string } | undefined): Promise<boolean> {
  ...
  if (accountPath && (req.method === "GET" || req.method === "POST")) {
    ...
    const [card] = await db.insert(cardsTable).values({ accountId: account.id, aesKey0: encrypt(k[0]), aesKey1: encrypt(k[1]), aesKey2: encrypt(k[2]), aesKey3: encrypt(k[3]), aesKey4: encrypt(k[4]), provisionToken: hash(token), provisionTokenExpiresAt: new Date(Date.now()+86400000) }).returning();
    return json(res, 201, { cardId: card.id, status: card.status, perTapLimitSats: card.perTapLimitSats, dailyLimitSats: card.dailyLimitSats, provisionUrl: `https://${DOMAIN}/api/provision/${token}`, lnurlwTemplate: `lnurlw://${DOMAIN}/card/${card.id}?p=${"0".repeat(32)}&c=${"0".repeat(16)}`, keys: Object.fromEntries(k.map((v,i)=>[`key${i}`,v])), createdAt: card.createdAt }) as never;
  }
