...
// Normal PN532 frames only. Validate the complete source before using any
// card-controlled length. The fixed packet buffer cannot hold extended frames.
static bool pn532_valid_frame(const uint8_t *frame, size_t available,
                              uint8_t response, size_t minimumLength) {
  if (!frame || available < 9 || frame[0] != 0 || frame[1] != 0 ||
      frame[2] != 0xff) {
    return false;
  }
  const size_t length = frame[3];
  if (length < minimumLength || length < 2 || length > available - 7 ||
      uint8_t(frame[3] + frame[4]) != 0 || frame[5] != 0xd5 ||
      frame[6] != response || frame[6 + length] != 0) {
    return false;
  }
  uint8_t checksum = 0;
...
static bool pn532_valid_iso_response(const uint8_t *frame, size_t available,
                                     size_t dataLength) {
  return pn532_valid_frame(frame, available, 0x41, 5) && frame[7] == 0 &&
         size_t(frame[3]) == dataLength + 5 &&
         frame[8 + dataLength] == 0x90 && frame[9 + dataLength] == 0x00;
}
...
bool Adafruit_PN532::readDetectedPassiveTargetID(uint8_t *uid,
                                                 uint8_t *uidLength) {
  if (!uidLength) return false;
  *uidLength = 0;
  if (!uid) return false;
  // read data packet
  readdata(pn532_packetbuffer, sizeof(pn532_packetbuffer));
  if (!pn532_valid_frame(pn532_packetbuffer, sizeof(pn532_packetbuffer),
                         0x4b, 8)) return false;
  // check some basic stuff
...
#endif
  if (pn532_packetbuffer[7] != 1 || pn532_packetbuffer[8] != 1)
    return 0;

  const uint8_t length = pn532_packetbuffer[12];
  // The public API promises a seven-byte destination, not an arbitrary UID.
  if ((length != 4 && length != 7) ||
      size_t(13) + length > size_t(5) + pn532_packetbuffer[3]) return false;
...
bool Adafruit_PN532::inDataExchange(uint8_t *send, uint8_t sendLength,
  ...
  const uint8_t capacity = *responseLength;
  *responseLength = 0;
  if (!send || !response || sendLength > PN532_PACKBUFFSIZ - 2) return false;
  pn532_packetbuffer[0] = PN532_COMMAND_INDATAEXCHANGE;
  pn532_packetbuffer[1] = _inListedTag;
  memcpy(pn532_packetbuffer + 2, send, sendLength);
  if (!sendCommandCheckAck(pn532_packetbuffer, sendLength + 2, 1000) ||
      !waitready(1000)) return false;
  readdata(pn532_packetbuffer, sizeof(pn532_packetbuffer));
  if (!pn532_valid_frame(pn532_packetbuffer, sizeof(pn532_packetbuffer),
                         PN532_RESPONSE_INDATAEXCHANGE, 3) ||
      (pn532_packetbuffer[7] & 0x3f) != 0) return false;
  const uint8_t length = pn532_packetbuffer[3] - 3;
...
uint8_t Adafruit_PN532::ntag424_apdu_send(
  ...
  uint8_t apdu[128];
  const size_t payload = comm_mode == NTAG424_COMM_MODE_FULL && cmd_data_length
  ...
  if (!cla || !ins || !p1 || !p2 || !response || !response_le ||
      ...
      comm_mode > NTAG424_COMM_MODE_FULL) return 0;
  uint8_t offset = 0;
  ...
  apdusize = offset;
  // No APDU payload logging: commands can contain card provisioning secrets.
  ...
  readdata(pn532_packetbuffer, sizeof(pn532_packetbuffer));
  if (!pn532_valid_frame(pn532_packetbuffer, sizeof(pn532_packetbuffer), 0x41, 3) ||
      pn532_packetbuffer[7] != 0 || pn532_packetbuffer[3] - 3 > response_le) return 0;
  ...
  if ((response_length >= 10) && (comm_mode == NTAG424_COMM_MODE_FULL)) {
    ...
    if (response_length > 10) {
      for (int i = response_length - 10 - 1; i >= 0; i--) {
        // Serial.println(i);
        if (response[i] == 0x00) {
          resp_no_padding = i;
...
uint8_t Adafruit_PN532::ntag424_ISOReadFile(uint8_t *buffer, int maxsize) {
  ...
  readdata(pn532_packetbuffer, sizeof(pn532_packetbuffer));
  // File settings may require authentication. Preserve that optional query,
  // but never continue after a corrupt frame or PN532 transport error.
  if (!pn532_valid_frame(pn532_packetbuffer, sizeof(pn532_packetbuffer),
                         0x41, 5) || pn532_packetbuffer[7] != 0) return 0;
#ifdef NTAG424DEBUG
  Adafruit_PN532::PrintHexChar(pn532_packetbuffer, 26);
  ...
  /* If byte 8 isn't 0x00 we probably have an error */
  if (!pn532_valid_iso_response(pn532_packetbuffer, 26, 0)) {
#ifdef NTAG424DEBUG
  ...
  pn532_packetbuffer[4] = 0x0;
  pn532_packetbuffer[5] = 0x0;
  pn532_packetbuffer[6] = 0x7; // NLEN plus the complete short URI header.
  ...
  /* Read the response packet */
  readdata(pn532_packetbuffer, 26);
  if (!pn532_valid_iso_response(pn532_packetbuffer, 26, 7)) return 0;
  const uint16_t nlen = (uint16_t(pn532_packetbuffer[8]) << 8) |
                        pn532_packetbuffer[9];
  // This API returns an uncompressed, single short URI record, not arbitrary
  // NDEF. Reject unsupported headers rather than returning a truncated URL.
  if (nlen < 6 || nlen - 5 > UINT8_MAX || nlen - 5 > maxsize ||
      pn532_packetbuffer[10] != 0xd1 || pn532_packetbuffer[11] != 1 ||
      uint16_t(pn532_packetbuffer[12]) != nlen - 4 ||
      pn532_packetbuffer[13] != 0x55 || pn532_packetbuffer[14] != 0) return 0;
  const uint16_t filesize = nlen - 5;
  ...
  const uint16_t pageCapacity = 32;
  const uint16_t pages = (filesize + pageCapacity - 1) / pageCapacity;
#ifdef NTAG424DEBUG
  PN532DEBUGPRINT.print("pages: ");
  PN532DEBUGPRINT.println(pages);
#endif
  for (uint16_t i = 0; i < pages; i++) {
    const uint16_t offset = i * pageCapacity;
    const uint8_t pagesize = (filesize - offset > pageCapacity)
                                ? pageCapacity : filesize - offset;
    ...
    pn532_packetbuffer[2] = NTAG424_COM_ISOCLA;
    pn532_packetbuffer[3] = NTAG424_CMD_ISOREADBINARY;
    pn532_packetbuffer[4] = (7 + offset) >> 8;
    pn532_packetbuffer[5] = (7 + offset) & 0xff;
...
#endif
    /* If byte 8 isn't 0x00 we probably have an error */
    if (pn532_valid_iso_response(pn532_packetbuffer, 64, pagesize)) {
...
void Adafruit_PN532::readdata(uint8_t *buff, uint8_t n) {
  ...
  if (spi_dev) {
    uint8_t cmd = PN532_SPI_DATAREAD;
    valid = spi_dev->write_then_read(&cmd, 1, buff, n);
  } else if (i2c_dev) {
    uint8_t rbuff[256] = {}; // uint8_t n plus the leading RDY byte.
    valid = i2c_dev->read(rbuff, size_t(n) + 1) && rbuff[0] == PN532_I2C_READY;
    if (valid) memcpy(buff, rbuff + 1, n);
  } else if (ser_dev) {
    valid = ser_dev->readBytes(buff, n) == n;
  }
  if (!valid) memset(buff, 0, n);
}
