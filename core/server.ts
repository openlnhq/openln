...
const json = (r: import("node:http").ServerResponse, s: number, b: unknown) => { r.writeHead(s, { "content-type": "application/json" }); r.end(JSON.stringify(b)); };
const server = createServer(async (req, res) => {
  ...
  if (req.method === "POST" && u.pathname === "/api/auth/register") { let b = ""; for await (const c of req) b += c; try { const v = JSON.parse(b) as { handle?: string; password?: string }; return json(res, 201, auth.register(v.handle ?? "", v.password ?? "")); } catch (e) { return json(res, 400, { error: e instanceof Error ? e.message : "Invalid request" }); } }
